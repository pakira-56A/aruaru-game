...
const resetGameState = {
  cards:     [],        // 生成されたカード要素を格納する配列
  flgFirst:  true,      // 1枚目のカードかどうかを示すフラグを初期化
  cardFirst: null,      // 1枚目にめくられたカードを保持する変数を初期化
  countUnit: 0,         // 見つけたペアの数
  currentColorIndex: 0, // どの色を次に使うかを追跡するためのインデックスを初期化
...
document.addEventListener("turbo:load", () => {
  ...
  else { console.warn('閉じるボタン見つかんない'); }
  ...
  if (postsDataElement) {
    const postID = postsDataElement.getAttribute("data-post-id");
    ...
    cardNumbers.forEach((number, index) => {
      ...
      panel.appendChild(card);             // 作成したカード要素を、パネルに追加
      gameState.cards.push(card); }); }    // 生成したカードを、カードの配列に追加
  else { console.warn('投稿データが見つかんない'); }
});
